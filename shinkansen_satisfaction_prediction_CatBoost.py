...
def train_and_validate_models(X_train, y_train, feature_names):
    ...
    models = {
        ...
        'CatBoost': CatBoostClassifier(random_state=42,loss_function='Logloss', eval_metric='Accuracy', verbose=False)
    }
...
def hyperparameter_tuning(X_train, y_train, best_model_name, best_model):
    ...
    if 'XGBoost' in best_model_name:
        ...
        optimized_model = CatBoostClassifier(**best_params,random_state=42, loss_function='Logloss', eval_metric='Accuracy', verbose=False)
    else:
